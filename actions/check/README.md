...
connecting them, grouped by branch. Conflicting migrations are highlighted (solid for this
branch's, outlined for ones already on the base branch) and annotated with the objects they touch;
sibling fork parents collapse into a single node and long runs of uninvolved migrations collapse
into a counter.
...
flowchart TD
    m0["add-status-enum"]
    subgraph basebranch["main"]
        m1["add-banned<br/>⚠ user_status"]
    end
    subgraph thisbranch["this branch"]
        m2["drop-inactive<br/>⚠ user_status"]
    end
    m0 --> m1
    m0 --> m2
...
A nested, marker-less rendering of the migration DAG, always present below the diagram (and
standalone when the diagram is off); every migration links to its folder at the checked commit.
The markers mirror the diagram: 🔴 conflicting migration from this branch, ⭕ conflicting
migration already on the base branch, ⚪ uninvolved migration on a conflicting chain:
